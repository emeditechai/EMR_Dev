...
public class PatientService(IDbConnectionFactory db) : IPatientService
{
    ...
    public async Task<IEnumerable<(int DoctorId, string FullName, int? PrimarySpecialityId, string Gender)>> GetOpdDoctorsAsync(int? branchId, int? departmentId = null, int? specialityId = null)
    {
        using var con = db.CreateConnection();
