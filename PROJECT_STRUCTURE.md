...
**Pathologist Approval Flow** (Settings → Pathologist Approval Flow, script `2093`, admin-only) — *configuration only so far*: a flow of 1–3 sequential approval levels with named pathologists per level, scoped per Company with optional Branch / Lab Department / one **or several** Test Categories (stored comma-separated in `Category_IDs`, e.g. `3,7,11`; precedence Category → Department → Branch → Company default; no two flows may overlap — a category belongs to one flow per branch, and there is one category-less flow per department). Tables `LabApprovalFlow` / `LabApprovalFlowLevel` / `LabApprovalFlowApprover`; API `api/lab-approval-flows`; Web `LabApprovalFlowsController` + `Views/LabApprovalFlows/`. Approvers must be active pathologists with a Registration No, working in the flow's branch, whose User Master **Assigned Department(s)** include the flow's department (strict) and whose **Test Categories (sign-off scope)** include every chosen category (empty scope = all categories). The same rule filters the level pickers and is re-checked on save; the pickers also list who was left out and why. `usp_LabApprovalFlow_Resolve` returns the flow that applies to a report but **no screen calls it yet** — the Approval screen, sign-off and print integration are a later step. Pathologists can also be limited to Test Categories in User Master (script `2092`, `Users.PathologistCategoryIds`) and can have a **signature image** (.png/.jpg/.jpeg, up to 2 MB, script `2097`, `Users.SignaturePath`): it is validated by file content, stored under `EMR.Web/App_Data/signatures` (outside `wwwroot`, git-ignored) and served only through the authorised `Users/Signature/{id}` action.
