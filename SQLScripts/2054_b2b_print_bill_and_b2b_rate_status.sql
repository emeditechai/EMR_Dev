-- ==============================================================================
-- Migration Script 2054: B2B Print Bill Partner Details & B2B Rate Status
-- 1. Updates dbo.usp_LabOrder_GetDetail to return AgentCode and compute
--    PaymentStatus and BalanceDue based on B2B Rate for B2B orders.
-- 2. Updates dbo.usp_LabOrder_GetPagedList to return AgentCode and compute
--    PaymentStatus and BalanceDue based on B2B Rate for B2B orders.
-- 3. Synchronizes historical PaymentHeader for Order 87.
-- ==============================================================================

SET NOCOUNT ON;
SET XACT_ABORT ON;

-- 1. Update dbo.usp_LabOrder_GetDetail
IF OBJECT_ID('dbo.usp_LabOrder_GetDetail', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_LabOrder_GetDetail;
GO
...
-- 2. Update dbo.usp_LabOrder_GetPagedList
IF OBJECT_ID('dbo.usp_LabOrder_GetPagedList', 'P') IS NOT NULL
    DROP PROCEDURE dbo.usp_LabOrder_GetPagedList;
GO
...
-- 3. Synchronize PaymentHeader for Order 87 (CityCare Franchise paid 300 in full on B2B total of 300)
UPDATE dbo.PaymentHeader
SET SubTotal = 300.00,
    NetAmount = 300.00,
    BalanceDue = 0.00,
    PaymentStatus = 'P'
WHERE ModuleCode = 'LAB' AND ModuleRefId = 87;
GO
