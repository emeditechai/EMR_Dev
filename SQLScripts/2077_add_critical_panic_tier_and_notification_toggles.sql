-- ============================================================
-- Script: 2077_add_critical_panic_tier_and_notification_toggles.sql
-- Description: Adds Notification_Required, Acknowledgement_Required, Tier, Low_Threshold, High_Threshold to LabReferenceRangeMaster
-- ============================================================
...
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.LabReferenceRangeMaster') AND name = 'Notification_Required')
BEGIN
    ALTER TABLE dbo.LabReferenceRangeMaster
    ADD Notification_Required BIT NOT NULL CONSTRAINT DF_LabRefRange_NotifReq DEFAULT 0;
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.LabReferenceRangeMaster') AND name = 'Acknowledgement_Required')
BEGIN
    ALTER TABLE dbo.LabReferenceRangeMaster
    ADD Acknowledgement_Required BIT NOT NULL CONSTRAINT DF_LabRefRange_AckReq DEFAULT 0;
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.LabReferenceRangeMaster') AND name = 'Tier')
BEGIN
    ALTER TABLE dbo.LabReferenceRangeMaster
    ADD Tier VARCHAR(50) NULL;
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.LabReferenceRangeMaster') AND name = 'Low_Threshold')
BEGIN
    ALTER TABLE dbo.LabReferenceRangeMaster
    ADD Low_Threshold DECIMAL(18,4) NULL;
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('dbo.LabReferenceRangeMaster') AND name = 'High_Threshold')
BEGIN
    ALTER TABLE dbo.LabReferenceRangeMaster
    ADD High_Threshold DECIMAL(18,4) NULL;
END
GO
...
CREATE OR ALTER PROCEDURE dbo.usp_Api_LabReferenceRange_Create
    ...
    @Low_Threshold             DECIMAL(18,4) = NULL,
    @High_Threshold            DECIMAL(18,4) = NULL,
    @Notification_Required     BIT = 0,
...
BEGIN
    ...
    IF @Low_Threshold IS NOT NULL AND @High_Threshold IS NOT NULL AND @Low_Threshold > @High_Threshold
    BEGIN
        RAISERROR('Low Threshold must be less than or equal to High Threshold.', 16, 1);
        RETURN;
    END
    ...
    VALUES (
        ...
        @Low_Threshold,
        @High_Threshold,
        ISNULL(@Notification_Required, 0),
        ISNULL(@Acknowledgement_Required, 0),
        @Special_Remarks,
        @Range_Source,
        @Effective_From,
        @Effective_To,
...
BEGIN
    ...
    SET 
        ...
        Notification_Required    = ISNULL(@Notification_Required, 0),
        Acknowledgement_Required = ISNULL(@Acknowledgement_Required, 0),
...
-- 6. Update usp_Api_LabReferenceRange_BulkSave
...
BEGIN
    ...
    BEGIN TRY
        ...
        WITH (
            ...
            Tier                VARCHAR(50)     '$.Tier',
            Low_Threshold       DECIMAL(18,4)   '$.Low_Threshold',
            High_Threshold      DECIMAL(18,4)   '$.High_Threshold',
