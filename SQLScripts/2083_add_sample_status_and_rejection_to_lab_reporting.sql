-- ============================================================
-- Script: 2083_add_sample_status_and_rejection_to_lab_reporting.sql
-- Description:
--   1. Add RejectionReasonId and RejectionReason columns to SampleCollection and labentrydetails
--   2. Create stored procedure dbo.usp_LabReporting_UpdateSampleStatus
--   3. Update dbo.usp_LabReporting_GetDetail to return sample status & rejection metadata
-- ============================================================

USE Dev_EMR;
GO

-- ── 1. Add columns to dbo.SampleCollection ──────────────────────────────────
IF NOT EXISTS (
    SELECT 1 FROM sys.columns 
    WHERE object_id = OBJECT_ID('dbo.SampleCollection') AND name = 'RejectionReasonId'
)
BEGIN
    ALTER TABLE dbo.SampleCollection ADD RejectionReasonId INT NULL;
    PRINT 'Added RejectionReasonId column to dbo.SampleCollection';
END
GO

IF NOT EXISTS (
    SELECT 1 FROM sys.columns 
    WHERE object_id = OBJECT_ID('dbo.SampleCollection') AND name = 'RejectionReason'
)
BEGIN
    ALTER TABLE dbo.SampleCollection ADD RejectionReason NVARCHAR(500) NULL;
    PRINT 'Added RejectionReason column to dbo.SampleCollection';
END
GO

-- ── 2. Add columns to dbo.labentrydetails ───────────────────────────────────
IF NOT EXISTS (
    SELECT 1 FROM sys.columns 
    WHERE object_id = OBJECT_ID('dbo.labentrydetails') AND name = 'RejectionReasonId'
)
BEGIN
    ALTER TABLE dbo.labentrydetails ADD RejectionReasonId INT NULL;
    PRINT 'Added RejectionReasonId column to dbo.labentrydetails';
END
GO

IF NOT EXISTS (
    SELECT 1 FROM sys.columns 
    WHERE object_id = OBJECT_ID('dbo.labentrydetails') AND name = 'RejectionReason'
)
BEGIN
    ALTER TABLE dbo.labentrydetails ADD RejectionReason NVARCHAR(500) NULL;
    PRINT 'Added RejectionReason column to dbo.labentrydetails';
END
GO

-- ── 3. Create dbo.usp_LabReporting_UpdateSampleStatus ───────────────────────
CREATE OR ALTER PROCEDURE dbo.usp_LabReporting_UpdateSampleStatus
    @LabOrderId         INT,
    @ProfileId          INT           = NULL,
    @InvestigationId    INT           = NULL,
    @SampleCollectionId BIGINT        = NULL,
    @CollectionStatusId INT,          -- 3 = RE_COLLECT, 4 = REJECTED, 2 = COLLECTED, 1 = PENDING
...
BEGIN
    ...
    -- Table variable to capture affected SampleCollectionIDs
    DECLARE @AffectedSamples TABLE (SampleCollectionId BIGINT);

    -- 1. Update SampleCollection
    UPDATE sc
    SET 
        sc.CollectionstatusID   = @CollectionStatusId,
        sc.RejectionReasonId    = @RejectionReasonId,
        sc.RejectionReason      = @RejectionReason,
        sc.Samplecollectiondate = CASE WHEN @CollectionStatusId IN (1, 3, 4) THEN NULL ELSE sc.Samplecollectiondate END,
        sc.Samplecollectiontime = CASE WHEN @CollectionStatusId IN (1, 3, 4) THEN NULL ELSE sc.Samplecollectiontime END,
        sc.ModifiedBy           = @UserId,
        sc.ModifiedDate         = @Now
    OUTPUT INSERTED.samplecollectionID INTO @AffectedSamples
    FROM dbo.SampleCollection sc
    WHERE sc.Laborderid = @LabOrderId
      AND sc.Is_Active = 1
      AND sc.Iscancelled = 0
      AND (
          (@SampleCollectionId IS NOT NULL AND sc.samplecollectionID = @SampleCollectionId)
          OR
          (@SampleCollectionId IS NULL AND @ProfileId IS NOT NULL AND sc.ProfileId = @ProfileId)
          OR
          (@SampleCollectionId IS NULL AND @ProfileId IS NULL AND @InvestigationId IS NOT NULL AND sc.InvestigationID = @InvestigationId)
      );

    -- 2. If rejected or re-collected, reset labentrydetails for those samples
    IF @CollectionStatusId IN (1, 3, 4)
    BEGIN
        UPDATE led
        SET 
            led.TestValue         = NULL,
            ...
            led.RejectionReasonId = @RejectionReasonId,
            led.RejectionReason   = @RejectionReason,
            led.ModifiedBy        = @UserId,
            led.ModifiedDate      = @Now
        FROM dbo.labentrydetails led
        INNER JOIN @AffectedSamples a ON a.SampleCollectionId = led.SamplecollectionID;
    END

    SELECT COUNT(1) AS Result FROM @AffectedSamples;
END;
GO

PRINT 'Created procedure dbo.usp_LabReporting_UpdateSampleStatus';
GO

-- ── 4. Update dbo.usp_LabReporting_GetDetail ────────────────────────────────
...
PRINT 'Updated dbo.usp_LabReporting_GetDetail with SampleStatus and Rejection metadata.';
GO
