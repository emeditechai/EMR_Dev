-- ============================================================================
-- Migration: 2101_lab_reporting_detail_branch_scope.sql
-- Description:
--   Lab Report Entry showed EVERY test of a bill on both branches of a partial sample transfer:
--   dbo.usp_LabReporting_GetDetail only took @LabOrderId, so it ignored which branch actually holds the sample
--   (dbo.usp_LabReporting_GetHeaderList already filtered correctly, which is why the bill appeared on both).
--
--   dbo.usp_LabReporting_GetDetail now takes an optional @BranchId and applies the SAME rule as the header list:
--       * a sample that was never transferred belongs to the branch that booked it (sc.BranchID)
--       * a transferred sample belongs to the TARGET branch, and only once it has been received (IsReceived = 1)
--
--   So on a partially transferred bill the source branch sees only the tests it kept, and the target branch sees
--   only the transferred tests it has received. The test rows, the progress counters / overall status and the
--   "transferred from" banner all use the same filter.
--
--   @BranchId = NULL keeps the old behaviour (every sample of the bill) - that is what the report PDF and the
--   Report Dispatch dashboard pass, so a printed report still carries the whole bill.
-- ============================================================================
...
PRINT 'Updated dbo.usp_LabReporting_GetDetail (branch scoped items).';
GO
