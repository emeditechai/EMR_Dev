-- ============================================================================
-- Migration: 2112_sample_transfer_ready_pending_report_only.sql
-- Description:
--   Sample Transfer > Dispatch Samples Worklist ("Ready" filter): a sample is only offered for dispatch while
--   its lab report has not been started yet (Report Status = Pending Entry - no dbo.labentrydetails row at all).
--   Once entry / validation / approval has begun at the source branch, the sample no longer appears here, since
--   transferring the physical specimen no longer makes sense against an already-worked report.
--
--   Applies to:
--     RS1 "ReadyCount"  - the READY FOR TRANSFER summary card, so it matches what the list actually shows.
--     RS2 detail rows   - only for @TransferStatusFilter = 'READY' (the Dispatch worklist itself).
--   The "Transferred" and "All" views are untouched - they are historical / audit views and must still show
--   every sample regardless of its report status.
-- ============================================================================
...
BEGIN
    ...
    -- Summary Stats
    SELECT 
        SUM(CASE WHEN ISNULL(sc.IsTransferred, 0) = 0 AND sc.CollectionstatusID = 2 AND led.LabEntryDetailId IS NULL THEN 1 ELSE 0 END) AS ReadyCount,
    ...
    WHERE sc.Is_Active = 1
      ...
      AND ISNULL(sc.IsoutSource, 0) = 0
      AND (
          (@NormFilter = 'READY' AND ISNULL(sc.IsTransferred, 0) = 0 AND sc.BranchID = @SourceBranchId AND led.LabEntryDetailId IS NULL)
...
PRINT 'Updated dbo.usp_SampleTransfer_GetEligibleSamples (Ready = Pending report only).';
GO
