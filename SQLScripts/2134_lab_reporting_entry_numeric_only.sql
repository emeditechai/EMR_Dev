-- ============================================================================
-- Script: 2134_lab_reporting_entry_numeric_only.sql
-- Description:
--   Ensures that only investigations whose Reporting Type in LabInvestigationMaster
--   is 'Numeric' (Master field 'Reporting Type *') are loaded and displayed on the
--   Lab Report Entry Page (and its corresponding worklist).
--
-- Updates:
--   1. dbo.usp_LabReporting_GetDetail:
--      - Adds @ReportingType NVARCHAR(50) = 'Numeric' (defaults to 'Numeric').
--      - Calculates order header counts & status based only on the requested reporting type.
--      - In RS 2, returns ISNULL(lim.Reporting_Type, 'Numeric') AS ReportingType and
--        filters items by (@ReportingType IS NULL OR lim.Reporting_Type = @ReportingType).
--   2. dbo.usp_LabReporting_GetHeaderList:
--      - Adds @ReportingType NVARCHAR(50) = 'Numeric' (defaults to 'Numeric').
--      - Filters EligibleOrders and ReportAggregates by @ReportingType so that the
--        Lab Reporting Entry worklist and item counts only track Numeric investigations.
-- ============================================================================
...
PRINT 'Updating dbo.usp_LabReporting_GetDetail for Numeric reporting type filtering...';
GO
...
CREATE OR ALTER PROCEDURE dbo.usp_LabReporting_GetDetail
    ...
    @ReportingType NVARCHAR(50) = 'Numeric' -- NULL = all; 'Numeric' = only Numeric tests (default for Numeric Report Entry)
AS
BEGIN
    SET NOCOUNT ON;
...
PRINT 'Updated dbo.usp_LabReporting_GetDetail (Numeric reporting type filtering).';
GO

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Update dbo.usp_LabReporting_GetHeaderList
-- ─────────────────────────────────────────────────────────────────────────────
PRINT 'Updating dbo.usp_LabReporting_GetHeaderList for Numeric reporting type filtering...';
GO
...
CREATE OR ALTER PROCEDURE dbo.usp_LabReporting_GetHeaderList
    ...
    @ReportingType   NVARCHAR(50)  = 'Numeric'
AS
BEGIN
    SET NOCOUNT ON;
           ...
           AND (
               ...
               OR EXISTS (
                   ...
                   WHERE sc_f.Laborderid = sc.Laborderid
                     ...
                     AND (@ReportingType IS NULL OR LOWER(LTRIM(RTRIM(ISNULL(lim_f.Reporting_Type, 'Numeric')))) = LOWER(LTRIM(RTRIM(@ReportingType))))
    ...
    ReportAggregates AS (
        ...
        LEFT JOIN dbo.labentrydetails led ON led.SamplecollectionID = sc.samplecollectionID AND led.IsActive = 1
        LEFT JOIN dbo.Reportentrystatus res ON res.ReportStatusId = led.ReportStatusId
        WHERE (@ReportingType IS NULL OR LOWER(LTRIM(RTRIM(ISNULL(lim.Reporting_Type, 'Numeric')))) = LOWER(LTRIM(RTRIM(@ReportingType))))
    ...
    SELECT 
        ...
        END AS ReportBadgeClass,
        ra.TotalEnteredCount,
        ra.DraftCount,
        ra.EntryCount,
        ra.ValidatedCount,
        ra.ApprovedCount,
        eo.HasTransferredSamples,
        eo.TransferredSourceBranchId,
        eo.TransferredDate
    INTO #FinalOrders
    FROM EligibleOrders eo
    INNER JOIN dbo.LabOrder lo ON lo.LabOrderId = eo.Laborderid
    INNER JOIN dbo.PatientMaster p ON p.PatientId = lo.PatientId
    LEFT JOIN ReportAggregates ra ON ra.Laborderid = eo.Laborderid
    WHERE (
        (@DateFilterType = 'BookingDate' AND ISNULL(lo.BookingDate, eo.MinBookingDateTime) BETWEEN @FromDate AND @ToDate)
    ...
    -- RS 1: Summary Statistics
    SELECT 
        COUNT(1) AS TotalCount,
        SUM(CASE WHEN ReportStatusName = 'Pending Entry' THEN 1 ELSE 0 END) AS PendingEntryCount,
        SUM(CASE WHEN ReportStatusName = 'Draft' THEN 1 ELSE 0 END) AS DraftCount,
        SUM(CASE WHEN ReportStatusName = 'Report Entry' THEN 1 ELSE 0 END) AS ReportEntryCount,
        SUM(CASE WHEN ReportStatusName = 'Report Validated' THEN 1 ELSE 0 END) AS ReportValidatedCount,
        SUM(CASE WHEN ReportStatusName = 'Report Approve' THEN 1 ELSE 0 END) AS ReportApproveCount
    FROM #FinalOrders;

    -- RS 2: Orders matching Status Filter
    SELECT *
    FROM #FinalOrders
    WHERE (
        @StatusFilter = 'All' 
        OR (@StatusFilter = 'Pending' AND ReportStatusName = 'Pending Entry')
        OR (@StatusFilter = 'Draft' AND ReportStatusName = 'Draft')
        OR (@StatusFilter = 'Entered' AND ReportStatusName = 'Report Entry')
        OR (@StatusFilter = 'Validated' AND ReportStatusName = 'Report Validated')
        OR (@StatusFilter = 'Approved' AND ReportStatusName = 'Report Approve')
    )
...
PRINT 'Updated dbo.usp_LabReporting_GetHeaderList (Numeric reporting type filtering).';
GO
