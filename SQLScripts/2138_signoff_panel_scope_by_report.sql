-- ============================================================================
-- Migration: 2138_signoff_panel_scope_by_report.sql
-- Description:
--   The signature block on a lab report named the wrong person whenever a bill held
--   tests of more than one department or both reporting types.
--
--   dbo.usp_Api_LabReport_GetSignoffPanel read every approval row of the bill and kept
--   one per level, ordered by approval time - the caller's @ReportingType / @DepartmentId
--   were only used for the configured-signatory case, never for the "who actually signed"
--   case. On bill LAB/HO2627000142 the Pathology test was approved at 14:29 and the
--   Radiology image test at 14:35, so the image report printed the pathologist; on
--   LAB/HO2627000140 the image test was approved first, so the numeric Pathology report
--   printed the radiologist.
--
--   Fix: the approvals considered are now limited to the tests actually on the report
--   being printed. With no @ReportingType and no @DepartmentId the scope stays open, so
...
PRINT 'Script 2138 applied: sign-off panel now scoped to the tests being printed.';
GO
